...
Vikings - Serie ainda em produção, que necessita de uma maior suspeição de descrença já que os resgistros históricos de seus personagens não são tão claros. É ambientada na era Viking no século IX, mostrando o estilo de vida e costumes nórdicos. A série é um sucesso de público.
...
Visualizando os valores por temporada, vemos que a primeira temporada de Vikings teve uma média e distribuição em posição bem inferior às demais séries, o que impacta em sua nota geral. Um dado curioso é a baixíssima dispersão de notas na segunda temporada de Vikings. Parece que todas as pessoas concordaram que a série mereceu um 8.6.
...
Ótimo relatório, Laybson. O que acrescentaria nas análises seria apenas um mapeamento do que cada forma representa. Por exemplo: no segundo gráfico os pontos representam o que? Assim alguém leigo (que não vai olhar no código pra entender) poderia entender melhor o gráfico. Interessantes as explicações.
